...
class ArrowJvmLocalScriptEngineFactory : KotlinJsr223JvmScriptEngineFactoryBase() {
  override fun getScriptEngine(): ScriptEngine =
    KotlinJsr223JvmLocalScriptEngine(
      this,
      scriptCompilationClasspathFromContextOrStdlib(classLoader = this.javaClass.classLoader, wholeClasspath = true)
        .filter { it.name.contains("kotlin") && !it.name.contains("stdlib") }.distinct(), // add classpath
      KotlinStandardJsr223ScriptTemplate::class.qualifiedName.orEmpty(),
      { ctx, types ->
        ScriptArgsWithTypes(
